...
Currently [mingster.com](https://mingster.com) is hosted at [vercel](https://vercel.com).

As you commit to main branch, new code will deploy to the stage automatically.

## Production

production site [mingster.com](https://mingster.com) is hosted at **mx2.mingster.com**, a
small (~4GB RAM) Debian box that **also runs [Mail-in-a-Box](https://mailinabox.email/)** and a
**local PostgreSQL 18**. Two facts drive the whole production setup:

1. **4GB RAM is tight for `next build`.** A default build OOMs (`spawn ENOMEM`, multi-GB `core.*`
   dumps), so the build runs on the box with a **capped Node heap** (`--max-old-space-size`) — no
   swap file needed. We build on the box (not ship a Mac-built `.next/`) to avoid Linux-vs-macOS
   native/bundle mismatches. See [Sync source, build on the box](#sync-source-build-on-the-box).
2. **Mail-in-a-Box owns nginx and TLS.** It regenerates `/etc/nginx/conf.d/local.conf` and
   renews Let's Encrypt certs on its own. Do **not** hand-edit nginx here or run your own
   certbot — your changes get wiped on the next MiaB update. Add the app the MiaB-blessed way.
   See [Serving the app behind Mail-in-a-Box](#serving-the-app-behind-mail-in-a-box).

> The generic single-purpose-VPS instructions further down (own nginx vhost on port 3001, own
> certbot) are **legacy / non-MiaB** and are kept only for reference. On mx2.mingster.com follow
> the two sections above.

---

## Sync source, build on the box

`web/bin/deploy.sh` runs from **your workstation**. It rsyncs the app **source** to the server,
then builds **on the box** with a capped Node heap (`--max-old-space-size`) — no swap file needed
on 4GB — and reloads PM2.

> **Why build on the box instead of shipping a prebuilt `.next/`?** The workstation is
> macOS/arm64, the server is Linux/x64, and a Next build is not portable across that gap:
> `node_modules` holds native binaries (the Prisma query engine most of all), and Next 16's default
> **Turbopack** externalizes `serverExternalPackages` (stripe, pino, pg, `@prisma/*`) with
> content-hash names that only resolve against the **exact** `node_modules` from build time. Ship
> the Mac build → native/hash mismatches → runtime crashes (`Failed to load external module
> stripe-<hash>`). Building on the box sidesteps all of it: Turbopack runs natively, the Prisma
> engine is Linux-native, and the hashes match the box's own `node_modules`.

`.env` stays on the server and is never synced or overwritten.

### First-time setup on the box

```bash
# runtimes (see full "Install Node.js…" and "PM2" sections below)
curl -fsSL https://deb.nodesource.com/setup_24.x | sudo bash - && sudo apt-get install -y nodejs
curl -fsSL https://bun.com/install | bash
bun install -g pm2

# app dir + your production .env (deploy.sh does the rest)
sudo mkdir -p /var/www/mingster.com/web
# copy your production .env into /var/www/mingster.com/web/.env
```

You also need SSH key access to the box (`ssh-copy-id`), since `deploy.sh` uses ssh/rsync.
...
```bash
MAX_OLD_SPACE=3072 web/bin/deploy.sh   # raise/lower the build heap cap (MB)
DB_PUSH=1          web/bin/deploy.sh   # also run `prisma db push` on the box (schema changed)
DRY_RUN=1          web/bin/deploy.sh   # rsync --dry-run; builds/reloads nothing
```

Config (override via env), with defaults:

| Var | Default | Purpose |
|-----|---------|---------|
| `DEPLOY_USER` | `root` | ssh user on the box |
| `DEPLOY_HOST` | `mx2.mingster.com` | server hostname |
| `DEPLOY_PATH` | `/var/www/mingster.com/web` | the `web/` dir on the box |
| `PM2_NAME` | `mingster.com` | pm2 process name |
| `SSH_PORT` | `22` | ssh port |
| `MAX_OLD_SPACE` | `2048` | Node heap cap (MB) for the build — the low-memory lever |
| `BUILD_CMD` | `bun run build` | build command run on the box |

```bash
# example: deploy as root over a non-standard ssh port
DEPLOY_USER=root SSH_PORT=2222 web/bin/deploy.sh
```

What the script does, in order: `rsync -az --delete` the **source** (excluding `node_modules`,
`.next`, `prisma/generated`, `.env`, `.git`, logs, `core.*`) → on the box `ulimit -c 0` (no core
dumps) → `bun install --frozen-lockfile` → `NODE_OPTIONS=--max-old-space-size=$MAX_OLD_SPACE
bun run build` (postinstall regenerates the Prisma client for Linux, then `next build`) →
`pm2 reload mingster.com` (or first-run `pm2 start`).

> If a build ever gets OOM-killed, raise `MAX_OLD_SPACE`, or add swap (see
> [`spawn ENOMEM` during `next build`](#spawn-enomem-during-next-build) below).

### Probe the box before/after

`web/bin/probe-server.sh` SSHes in and prints RAM/swap, listening ports, the MiaB nginx
`server_name` list, runtimes, postgres, and the app's PM2 status. Read-only — changes nothing.
...
## Serving the app behind Mail-in-a-Box

Mail-in-a-Box already terminates TLS on 80/443 and issues the Let's Encrypt cert for
`mingster.com` (because it's a mail domain on the box). We just tell MiaB to **reverse-proxy the
domain root to the app** on `localhost:3002` (the port `bun start` / `next start -p 3002` uses).
Both files below live under `STORAGE_ROOT` (`/home/user-data`) and **survive MiaB updates** —
MiaB reads them when it regenerates nginx.

**1. Proxy the root** — edit `/home/user-data/www/custom.yaml`:

```yaml
mingster.com:
  proxies:
    "/": "http://localhost:3002#web-sockets,pass-http-host,no-proxy-redirect"
```

The flags make MiaB emit the right directives: `web-sockets` → HTTP/1.1 + `Upgrade` headers;
`pass-http-host` → `Host $http_host` (needed for Better Auth / OAuth callbacks);
`no-proxy-redirect` → `proxy_redirect off`. MiaB always adds `X-Forwarded-For/-Host/-Proto` and
`X-Real-IP`. A root (`/`) proxy also makes MiaB stop serving its static placeholder for the domain.

**2. Longer timeouts + upload size** — create `/home/user-data/www/mingster.com.conf`
(MiaB includes this file inside the `mingster.com` server block; server-context directives here
are inherited by the proxied `location /`):

```nginx
# app can take a while on some POST paths; allow bigger uploads
...
**3. Apply** (regenerates nginx, keeps your two files, reloads):

```bash
cd /root/mailinabox
sudo tools/web_update      # or: Admin panel → System → reload; or `sudo mailinabox`
```

Verify:

```bash
grep -A3 'server_name mingster.com' /etc/nginx/conf.d/local.conf   # should show your proxy_pass
curl -I https://mingster.com                                       # 200 from Next.js, valid TLS
```

> If `mingster.com` is **not** yet a mail domain on the box, MiaB won't create a server block or
> cert for it. Add it in the MiaB admin (Mail → Users, or a domain alias) first, or point the
> domain's A record at the box so MiaB provisions the cert.
...
```bash
sudo hostnamectl set-hostname mingster.com
```
...
> **On mx2.mingster.com, skip this section** — Mail-in-a-Box owns nginx. Use
> [Serving the app behind Mail-in-a-Box](#serving-the-app-behind-mail-in-a-box) instead. The vhost
> below is for a **standalone (non-MiaB) VPS** only, and note the app listens on **3002** now, not
> 3001.

```bash
sudo apt install nginx -y

sudo nano /etc/nginx/sites-available/mingster.com
```
...
server {
    ...
    location / {
        ...
        # OAuth callbacks + AI menu scan (Gemini can run several minutes)
        proxy_connect_timeout 300s;
        proxy_send_timeout 300s;
        proxy_read_timeout 300s;
    }
...
```bash
sudo ln -s /etc/nginx/sites-available/mingster.com /etc/nginx/sites-enabled/
```
...
**Existing production servers:** If AI menu scan works locally but fails on `mingster.com` with 502/504 or a dropped connection after ~60 seconds, raise the three `proxy_*_timeout` values above (300s is enough for Gemini Pro scans) and reload nginx.

**Store admin realtime nav (SSE):** After deploying sidebar badge SSE, add the dedicated nginx block and env guidance in [[DEPLOYMENT-STORE-ADMIN-REALTIME-NAV]]. Architecture: [[STORE-ADMIN-REALTIME-NAV]]. **PM2 cluster or multiple app servers:** enable Redis first — [[DEPLOYMENT-redis]].

#### SSL

> **On mx2.mingster.com, skip this** — Mail-in-a-Box issues and renews the Let's Encrypt cert for
> `mingster.com` automatically. Running your own certbot will fight with MiaB. The commands below
> are for a **standalone (non-MiaB) VPS** only.

```bash
sudo apt install certbot python3-certbot-nginx -y

sudo certbot --nginx -d mingster.com
```
...
> On mx2.mingster.com you do **not** need to clone — `web/bin/deploy.sh` rsyncs the build and
> source from your workstation into `/var/www/mingster.com/web`. A clone is only useful if you
> want the cron scripts / repo present independently. The build itself never happens on the box.

```bash
sudo mkdir -p /var/www
cd /var/www
sudo  git clone https://github.com/mingster/mingster.com.git
```
...
**Server Action error after deploy:** If you see `Failed to find Server Action "x"` in PM2 logs, add a persistent encryption key so action IDs stay valid across deploys. Generate once and put in production `.env`:
...
```bash
NEXT_SERVER_ACTIONS_ENCRYPTION_KEY=<paste the output above>
```

See [ENVIRONMENT_VARIABLES.md](../ENVIRONMENT_VARIABLES.md#nextjs-server-actions-self-hosted--pm2) for details.
...
Deploys are driven by `web/bin/deploy.sh` (sync source → build on the box with a capped heap →
reload PM2). See [Sync source, build on the box](#sync-source-build-on-the-box). The box builds the
app itself; the notes below help when a build runs low on memory.
...
- **Free space now:** Remove them only after you are sure you do not need them for debugging:
  `sudo find /var/www/mingster.com -maxdepth 3 -name 'core.*' -type f -delete`
...
On the server, from `/var/www/mingster.com/web`:
...
#mkdir -p /var/www/mingster.com
cd /var/www/mingster.com/web

# `--cwd` is required: `bun start` must run in the repo (package.json "start" script).
#pm2 delete mingster.com   # if fixing a broken process
pm2 start bun --name "mingster.com" --cwd /var/www/mingster.com/web -- start
...
# 7. Check the logs
pm2 logs mingster.com --lines 50
```
...
The script calls: `curl -H "Authorization: Bearer ${CRON_SECRET}" https://mingster.com/api/cron-jobs/sendmail?batchSize=10&maxConcurrent=3`
...
2. **Add to crontab:**
   ...
   ```bash
   * * * * * . ~/.bashrc && /var/www/mingster.com/web/bin/run-sendmail-cron.sh >> /var/log/sendmail.log 2>&1
   * * * * * sleep 10; . ~/.bashrc && /var/www/mingster.com/web/bin/run-sendmail-cron.sh >> /var/log/sendmail.log 2>&1
   * * * * * sleep 20; . ~/.bashrc && /var/www/mingster.com/web/bin/run-sendmail-cron.sh >> /var/log/sendmail.log 2>&1
   * * * * * sleep 30; . ~/.bashrc && /var/www/mingster.com/web/bin/run-sendmail-cron.sh >> /var/log/sendmail.log 2>&1
   * * * * * sleep 40; . ~/.bashrc && /var/www/mingster.com/web/bin/run-sendmail-cron.sh >> /var/log/sendmail.log 2>&1
   * * * * * sleep 50; . ~/.bashrc && /var/www/mingster.com/web/bin/run-sendmail-cron.sh >> /var/log/sendmail.log 2>&1
   ```

   Or to customize batch size and concurrency:

   ```bash
   * * * * * . ~/.bashrc && BATCH_SIZE=20 MAX_CONCURRENT=5 /var/www/mingster.com/web/bin/run-sendmail-cron.sh >> /var/log/sendmail.log 2>&1
   * * * * * sleep 10; . ~/.bashrc && BATCH_SIZE=20 MAX_CONCURRENT=5 /var/www/mingster.com/web/bin/run-sendmail-cron.sh >> /var/log/sendmail.log 2>&1
   * * * * * sleep 20; . ~/.bashrc && BATCH_SIZE=20 MAX_CONCURRENT=5 /var/www/mingster.com/web/bin/run-sendmail-cron.sh >> /var/log/sendmail.log 2>&1
   * * * * * sleep 30; . ~/.bashrc && BATCH_SIZE=20 MAX_CONCURRENT=5 /var/www/mingster.com/web/bin/run-sendmail-cron.sh >> /var/log/sendmail.log 2>&1
   * * * * * sleep 40; . ~/.bashrc && BATCH_SIZE=20 MAX_CONCURRENT=5 /var/www/mingster.com/web/bin/run-sendmail-cron.sh >> /var/log/sendmail.log 2>&1
   * * * * * sleep 50; . ~/.bashrc && BATCH_SIZE=20 MAX_CONCURRENT=5 /var/www/mingster.com/web/bin/run-sendmail-cron.sh >> /var/log/sendmail.log 2>&1
   ```
...
3. **Test the API endpoint manually:**
   ...
   ```bash
   source ~/.bashrc
   curl -X GET https://mingster.com/api/cron-jobs/sendmail \
...
The script calls: `curl -X GET "https://mingster.com/api/cron-jobs/process-notification-queue?batchSize=100" -H "Authorization: Bearer ${CRON_SECRET}"`
...
2. **Add to crontab:**
   ...
   ```bash
   */2 * * * * . ~/.bashrc && /var/www/mingster.com/web/bin/run-process-notification-queue-cron.sh >> /var/log/process-notification-queue.log 2>&1
   ```
...
3. **Test the API endpoint manually:**
   ...
   ```bash
   source ~/.bashrc
   curl -X GET "https://mingster.com/api/cron-jobs/process-notification-queue?batchSize=100" \
     -H "Authorization: Bearer ${CRON_SECRET}"
   ```
...
The script calls: `curl -H "Authorization: Bearer ${CRON_SECRET}" https://mingster.com/api/cron-jobs/cleanup-unpaid-rsvps?ageMinutes=5`
...
2. **Add to crontab:**
   ...
   ```bash
   */5 * * * * . ~/.bashrc && /var/www/mingster.com/web/bin/run-cleanup-unpaid-rsvps-cron.sh >> /var/log/cleanup-unpaid-rsvps.log 2>&1
   ```
   ...
   ```bash
   */5 * * * * . ~/.bashrc && AGE_MINUTES=10 /var/www/mingster.com/web/bin/run-cleanup-unpaid-rsvps-cron.sh >> /var/log/cleanup-unpaid-rsvps.log 2>&1
   ```
...
3. **Test the API endpoint manually:**
   ...
   ```bash
   source ~/.bashrc
   curl -X GET https://mingster.com/api/cron-jobs/cleanup-unpaid-rsvps \
     -H "Authorization: Bearer ${CRON_SECRET}"
   ```
...
2. **Add to crontab:**
   ...
   ```bash
   */10 * * * * . ~/.bashrc && /var/www/mingster.com/web/bin/run-rsvp-reminders-cron.sh >> /var/log/rsvp-reminders.log 2>&1
   ```
...
3. **Test the API endpoint manually:**
   ...
   ```bash
   source ~/.bashrc
   curl -X GET https://mingster.com/api/cron-jobs/process-reminders \
     -H "Authorization: Bearer ${CRON_SECRET}"
   ```
...
```bash
curl -X GET https://mingster.com/api/cron-jobs/process-rsvp-customer-confirm \
  -H "Authorization: Bearer ${CRON_SECRET}"
```
...
```bash
*/10 * * * * . ~/.bashrc && /var/www/mingster.com/web/bin/run-rsvp-customer-confirm-cron.sh >> /var/log/rsvp-customer-confirm.log 2>&1
```
...
The script calls: `curl -X GET https://mingster.com/api/cron-jobs/sync-delivery-status -H "Authorization: Bearer ${CRON_SECRET}"`
...
3. **Add to crontab:**
   ...
   ```bash
   0 * * * * . ~/.bashrc && /var/www/mingster.com/web/bin/run-sync-delivery-status-cron.sh >> /var/log/sync-delivery-status.log 2>&1
   ```

   Or if you prefer every 3 minutes:

   ```bash
   */3 * * * * . ~/.bashrc && /var/www/mingster.com/web/bin/run-sync-delivery-status-cron.sh >> /var/log/sync-delivery-status.log 2>&1
   ```
...
4. **Test the API endpoint manually:**
   ...
   ```bash
   source ~/.bashrc
   curl -X GET https://mingster.com/api/cron-jobs/sync-delivery-status \
     -H "Authorization: Bearer ${CRON_SECRET}"
   ```
...
* * * * * . ~/.bashrc && /var/www/mingster.com/web/bin/run-process-notification-queue-cron.sh >> /var/log/process-notification-queue.log 2>&1
* * * * * sleep 10; . ~/.bashrc && /var/www/mingster.com/web/bin/run-process-notification-queue-cron.sh >> /var/log/process-notification-queue.log 2>&1
* * * * * sleep 20; . ~/.bashrc && /var/www/mingster.com/web/bin/run-process-notification-queue-cron.sh >> /var/log/process-notification-queue.log 2>&1
* * * * * sleep 30; . ~/.bashrc && /var/www/mingster.com/web/bin/run-process-notification-queue-cron.sh >> /var/log/process-notification-queue.log 2>&1
* * * * * sleep 40; . ~/.bashrc && /var/www/mingster.com/web/bin/run-process-notification-queue-cron.sh >> /var/log/process-notification-queue.log 2>&1
* * * * * sleep 50; . ~/.bashrc && /var/www/mingster.com/web/bin/run-process-notification-queue-cron.sh >> /var/log/process-notification-queue.log 2>&1

# Cleanup unpaid RSVPs older than 30 minutes (every 5 minutes)
*/5 * * * * . ~/.bashrc && /var/www/mingster.com/web/bin/run-cleanup-unpaid-rsvps-cron.sh >> /var/log/cleanup-unpaid-rsvps.log 2>&1

# Process RSVP reminder notifications (every 5 minutes)
*/5 * * * * . ~/.bashrc && /var/www/mingster.com/web/bin/run-rsvp-reminders-cron.sh >> /var/log/rsvp-reminders.log 2>&1
...
*/5 * * * * . ~/.bashrc && /var/www/mingster.com/web/bin/run-sync-delivery-status-cron.sh >> /var/log/sync-delivery-status.log 2>&1
