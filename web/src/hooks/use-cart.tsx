...
function reducer(state: CartProviderState, action: Actions) {
	switch (action.type) {
		case "SET_ITEMS":
			return generateCartState(action.payload, state);
...
const generateCartState = (items: Item[], state = initialState) => {
...
//const calculateTotal = (items: Item[]) => items.reduce((total, item) => total + item.quantity! * item.price, 0);
...
export const CartProvider: React.FC<{
	...
	const [state, dispatch] = React.useReducer(
		reducer,
		JSON.parse(savedCart as string),
	);
	React.useEffect(() => {
		if (saveCart) {
